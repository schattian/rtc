...
func TestFabric_Produce(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			isCreated := thelper.IOExist(t, mFs, tt.fabric.Dir, afero.DirExists)
			...
			for _, table := range tt.fabric.Schema.Blueprint {
				...
				got := thelper.IOReadFile(t, mFs, tt.fabric.Dir+"/"+generatedFilename)
				thelper.CmpWithGoldenFile(t, got, fmt.Sprintf("fabric/%s", tt.product[table.Name]), "Fabric.Produce()")
			}
		})
	}
}
